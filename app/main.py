...
async def forward(request: Request, db: Session=Depends(get_db)):
    ...
    hist = RequestHistory(path='/forward', status_code=int(status_code), duration_ms=float(duration_ms), headers_json=_json_dumps_safe(dict(request.headers)), body_json=_json_dumps_safe(payload), response_json=_json_dumps_safe(response_obj) if not isinstance(response_obj, str) else response_obj, error_text=error_text)
    db.add(hist)
    db.commit()
...
def history(limit: int=200, offset: int=0, db: Session=Depends(get_db)):
    ...
    out = []
    for r in rows:
        out.append({'id': r.id, 'created_at': r.created_at.isoformat(), 'path': r.path, 'status_code': r.status_code, 'duration_ms': r.duration_ms, 'headers': _safe_json_loads(r.headers_json) if r.headers_json else {}, 'body': _safe_json_loads(r.body_json) if r.body_json else None, 'response': _safe_json_loads(r.response_json) if r.response_json else None, 'error': r.error_text or ''})
    return {'items': out, 'limit': limit, 'offset': offset, 'count': len(out)}

@app.get('/stats')
def stats(db: Session=Depends(get_db)):
    stmt = select(RequestHistory.status_code, RequestHistory.duration_ms, RequestHistory.body_json).where(RequestHistory.path == '/forward')
    ...
    for r in rows:
        ...
        try:
            ...
            num_features_list.append(0)
            missing_list.append(len(expected) if expected else 0)
    return {'requests_total': total, 'requests_success': len(success_rows), 'requests_failed': failed_rows, 'expected_raw_cols': sorted(list(expected)), 'latency_ms_all': _quantiles(durations_all), 'latency_ms_success': _quantiles(durations_ok), 'input_num_features': _quantiles(np.array(num_features_list, dtype=float)), 'input_missing_features': _quantiles(np.array(missing_list, dtype=float)), 'input_payload_bytes': _quantiles(np.array(payload_bytes_list, dtype=float))}