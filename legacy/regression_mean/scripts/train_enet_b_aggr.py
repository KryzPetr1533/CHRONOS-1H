...
def time_split(df: pd.DataFrame, train_frac: float, val_frac: float, test_frac: float) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    assert abs(train_frac + val_frac + test_frac - 1.0) < 1e-09, 'fractions must sum to 1.0'
...
def regression_metrics(y_true: np.ndarray, y_pred: np.ndarray) -> Dict[str, float]:
    ...
    return {'rmse': rmse, 'mae': mae, 'nrmse': rmse / std if std and (not np.isnan(std)) else np.nan, 'mae_over_std': mae / std if std and (not np.isnan(std)) else np.nan, 'r2': r2, 'corr': corr, 'directional_accuracy': dir_acc}
...
def main(cfg: DictConfig) -> None:
    ...
    df_train, df_val, df_test = time_split(df, float(cfg.split.train_frac), float(cfg.split.val_frac), float(cfg.split.test_frac))
    ...
    estimator = Pipeline([('fe', ChronosFeatureEngineerB(required_raw=raw_cols, output_cols=list(cfg.columns.output_cols))), ('scaler', StandardScaler()), ('model', ElasticNet(max_iter=10000))])
    param_grid = {'model__alpha': list(cfg.grid.alpha), 'model__l1_ratio': list(cfg.grid.l1_ratio)}
    cv = TimeSeriesSplit(n_splits=int(cfg.split.cv_splits))
    gcv = GridSearchCV(estimator=estimator, param_grid=param_grid, scoring='neg_mean_squared_error', cv=cv, n_jobs=int(cfg.n_jobs), refit=True)
    gcv.fit(X_train, y_train)
    best_model: Pipeline = gcv.best_estimator_
    y_val_pred = best_model.predict(X_val)
    y_test_pred = best_model.predict(X_test)
    metrics: Dict[str, Any] = {'best_params': gcv.best_params_, 'cv_best_score_neg_mse': float(gcv.best_score_), 'val': regression_metrics(y_val, y_val_pred), 'test': regression_metrics(y_test, y_test_pred), 'n_rows': {'train': int(len(df_train)), 'val': int(len(df_val)), 'test': int(len(df_test))}, 'ts_range': {'min': str(df['ts'].min()), 'max': str(df['ts'].max())}}
    ...
    meta = {'model_path': str(model_path), 'raw_cols': raw_cols, 'output_cols': list(cfg.columns.output_cols), 'target': 'target_log_ret_1h'}
