...
def make_datasets(df: pd.DataFrame):
    ...
    training = TimeSeriesDataSet(
        ...
        static_categoricals=[],
        static_reals=[],
        time_varying_known_reals=["time_idx", "hour_sin", "hour_cos", "dow_sin", "dow_cos"],
...
def main() -> None:
    ...
    search_space = [
        {"learning_rate": 3e-4, "hidden_size": 64,  "n_heads": 4, "e_layers": 2, "d_ff": 256,  "dropout": 0.2, "patch_length": 24},
        {"learning_rate": 1e-3, "hidden_size": 128, "n_heads": 4, "e_layers": 2, "d_ff": 512,  "dropout": 0.2, "patch_length": 24},
        {"learning_rate": 3e-4, "hidden_size": 128, "n_heads": 8, "e_layers": 3, "d_ff": 512,  "dropout": 0.2, "patch_length": 24},
        {"learning_rate": 1e-3, "hidden_size": 256, "n_heads": 8, "e_layers": 2, "d_ff": 1024, "dropout": 0.1, "patch_length": 24},
        {"learning_rate": 3e-4, "hidden_size": 128, "n_heads": 4, "e_layers": 2, "d_ff": 512,  "dropout": 0.3, "patch_length": 12},
        {"learning_rate": 3e-4, "hidden_size": 128, "n_heads": 4, "e_layers": 2, "d_ff": 512,  "dropout": 0.2, "patch_length": 48},
    ]
    ...
    for i, cfg in enumerate(search_space, start=1):
        ...
        if val_metrics["rmse"] < best_val_rmse:
            ...
            best = {
                ...
                "baseline_val": regression_metrics(y_val, baseline_val),
                "baseline_test": regression_metrics(y_test, baseline_test),
                "y_test": y_test,
                "p_test": p_test,
                "baseline_test_pred": baseline_test,
            }

    pd.DataFrame(search_rows).sort_values("val_rmse").to_csv(
        OUT_DIR / "timexer_search_results.csv", index=False
    )
    ...
    pd.DataFrame(
        {
            "y_true": best["y_test"],
            "y_pred": best["p_test"],
            "y_pred_baseline": best["baseline_test_pred"],
        }
    ).to_csv(OUT_DIR / "timexer_test_predictions.csv", index=False)
    ...
    result = {
        ...
        "baseline": {
            "val": best["baseline_val"],
            "test": best["baseline_test"],
        },
        "timexer": {
    ...
    (OUT_DIR / "timexer_metrics.json").write_text(
