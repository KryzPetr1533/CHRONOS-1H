...
def main() -> None:
    ...
    model = PatchTST(h=H, input_size=INPUT_SIZE, hist_exog_list=HIST_EXOG, futr_exog_list=FUTR_EXOG, encoder_layers=3, n_heads=8, hidden_size=128, linear_hidden_size=256, dropout=0.1, loss=MAE(), valid_loss=MAE(), scaler_type='robust', max_steps=1500, val_check_steps=100, early_stop_patience_steps=5, batch_size=256, windows_batch_size=1024, random_seed=42, enable_progress_bar=True, logger=False)
    nf = NeuralForecast(models=[model], freq='H')
    cv_df = nf.cross_validation(df=df, val_size=VAL_SIZE, test_size=TEST_SIZE, step_size=1, n_windows=None, refit=False, verbose=1)
    ...
    result = {'data_csv': str(DATA_CSV), 'model': 'PatchTST', 'h': H, 'input_size': INPUT_SIZE, 'hist_exog': HIST_EXOG, 'futr_exog': FUTR_EXOG, 'n_predictions': int(len(cv_df)), 'ts_range': {'min': str(cv_df['ds'].min()), 'max': str(cv_df['ds'].max())}, 'baseline': regression_metrics(y_true, y_base), 'patchtst': regression_metrics(y_true, y_pred)}
    cv_df.to_csv(OUT_DIR / 'patchtst_cv_predictions.csv', index=False)
    (OUT_DIR / 'patchtst_metrics.json').write_text(json.dumps(result, ensure_ascii=False, indent=2), encoding='utf-8')
