...
def interval_to_ms(interval: str) -> int:
    mapping = {'1m': 60000, '3m': 180000, '5m': 300000, '15m': 900000, '30m': 1800000, '1h': 3600000, '2h': 7200000, '4h': 14400000, '6h': 21600000, '8h': 28800000, '12h': 43200000, '1d': 86400000}
...
def make_session() -> requests.Session:
    s = requests.Session()
    retry_strategy = Retry(total=5, backoff_factor=0.5, status_forcelist=[429, 500, 502, 503, 504], allowed_methods=['GET'], raise_on_status=False)
...
class BinancePublic:
    ...
    @retry(wait=wait_exponential(multiplier=1, min=1, max=60), stop=stop_after_attempt(7), retry=retry_if_exception_type(requests.RequestException))
    ...
    def fetch_klines(self, symbol: str, interval: str, start: datetime, end: Optional[datetime]=None, limit: int=1500) -> pd.DataFrame:
        ...
        while True:
            ...
            if next_open > end_ms:
                break
            start_ms = next_open
        cols = ['open_time', 'open', 'high', 'low', 'close', 'volume', 'close_time', 'quote_volume', 'num_trades', 'taker_buy_base', 'taker_buy_quote', 'ignore']
    ...
    def fetch_premium_index_klines(self, symbol: str, interval: str, start: datetime, end: Optional[datetime]=None, limit: int=1500) -> pd.DataFrame:
        url = f'{FAPI_BASE}/fapi/v1/premiumIndexKlines'
        end = end or datetime.now(timezone.utc)
        start_ms, end_ms = (to_ms(start), to_ms(end))
        out: List[List[Any]] = []
        while True:
            data = self._get(url, {'symbol': symbol, 'interval': interval, 'startTime': start_ms, 'endTime': end_ms, 'limit': limit})
            if not data:
                break
            out.extend(data)
            last = data[-1][0]
    ...
    def fetch_funding_rates(self, symbol: str, start: datetime, end: Optional[datetime]=None, limit: int=1000) -> pd.DataFrame:
    ...
    def fetch_taker_buy_sell_volume(self, symbol: str, period: str='1h', limit: int=500, start: Optional[datetime]=None, end: Optional[datetime]=None) -> pd.DataFrame:
    ...
    def fetch_taker_long_short_ratio(self, symbol: str, period: str='1h', limit: int=500, start: Optional[datetime]=None, end: Optional[datetime]=None) -> pd.DataFrame:
    ...
    def fetch_open_interest_hist(self, symbol: str, period: str='1h', limit: int=500, start: Optional[datetime]=None, end: Optional[datetime]=None) -> pd.DataFrame:
    ...
    def fetch_vision_monthly_klines_1h(self, symbol: str, start_month: str, end_month: Optional[str]=None) -> pd.DataFrame:
        import io, zipfile

        def month_iter(start_m: str, end_m: Optional[str]):
            y, m = map(int, start_m.split('-'))
            end_dt = datetime.now(timezone.utc) if end_m is None else datetime.strptime(end_m, '%Y-%m').replace(tzinfo=timezone.utc)
            ...
            try:
                ...
                with zipfile.ZipFile(io.BytesIO(resp.content)) as zf:
                    ...
                    with zf.open(csv_names[0]) as fh:
                        ...
                        df.columns = ['open_time', 'open', 'high', 'low', 'close', 'volume', 'close_time', 'quote_volume', 'num_trades', 'taker_buy_base', 'taker_buy_quote', 'ignore']
                        for c in ['open', 'high', 'low', 'close', 'volume', 'quote_volume', 'taker_buy_base', 'taker_buy_quote']:
                            df[c] = pd.to_numeric(df[c], errors='coerce')
                        df['open_time'] = pd.to_datetime(df['open_time'], unit='ms', utc=True)
                        df['close_time'] = pd.to_datetime(df['close_time'], unit='ms', utc=True)
                ...
                print(f'Skip {url}: {e}')
        if not frames:
            return pd.DataFrame(columns=['open_time', 'open', 'high', 'low', 'close', 'volume', 'close_time', 'quote_volume', 'num_trades', 'taker_buy_base', 'taker_buy_quote', 'ignore'])
