...
def evaluate_predictions(y_true: np.ndarray, y_pred: np.ndarray) -> dict[str, float]:
    ...
    metrics = PredictionMetrics(
        rmse=rmse,
        mae=mae,
        nrmse=float(rmse / std) if np.isfinite(std) and std > 0 else float("nan"),
        mae_over_std=float(mae / std) if np.isfinite(std) and std > 0 else float("nan"),
        r2=float(r2_score(y_true, y_pred)),
        pearson_corr=_safe_corr(y_true, y_pred, method="pearson"),
        spearman_corr=_safe_corr(y_true, y_pred, method="spearman"),
        directional_accuracy=directional_accuracy,
        top20_directional_accuracy=top20_directional_accuracy,
        sign_strategy_mean=float(np.mean(pnl)),
        sign_strategy_sharpe=sign_strategy_sharpe,
    )
