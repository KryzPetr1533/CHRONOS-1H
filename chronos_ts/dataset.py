...
class DatasetBuildConfig:
    ...
    def core_source_cols(self) -> list[str]:
        return ['close', 'volume', 'quote_volume', 'num_trades', 'taker_buy_base', 'taker_buy_quote', 'premium_close', 'fundingRate', 'log_ret_1h']

    @property
    def rich_extra_source_cols(self) -> list[str]:
        return ['buyVol', 'sellVol', 'buySellRatio', 'taker_imbalance', 'sumOpenInterest', 'sumOpenInterestValue']
...
class ExperimentDatasetBuilder:
    ...
    def _lag_feature_candidates(self, df: pd.DataFrame) -> list[str]:
        candidates = ['log_ret_1h', 'premium_close', 'premium_chg', 'fundingRate', 'mark_minus_close', 'volume', 'quote_volume', 'num_trades', 'taker_buy_share', 'vol_chg', 'trades_chg', 'taker_imbalance', 'buy_share', 'net_taker_flow', 'sumOpenInterest', 'sumOpenInterestValue', 'oi_pct', 'avg_open_price', 'oi_to_volume']
        return [c for c in candidates if c in df.columns]

    def _rolling_feature_candidates(self, df: pd.DataFrame) -> list[str]:
        candidates = ['log_ret_1h', 'premium_close', 'premium_chg', 'fundingRate', 'volume', 'quote_volume', 'num_trades', 'taker_buy_share', 'taker_imbalance', 'oi_pct', 'vol_chg', 'trades_chg']
        return [c for c in candidates if c in df.columns]
    ...
    def _final_columns(self, df: pd.DataFrame) -> list[str]:
        ...
        derived_candidates = ['mark_minus_close', 'taker_buy_share', 'premium_chg', 'oi_pct', 'vol_chg', 'trades_chg', 'buy_share', 'net_taker_flow', 'avg_open_price', 'oi_to_volume', 'hour_sin', 'hour_cos', 'dow_sin', 'dow_cos', 'fund_cycle_sin', 'fund_cycle_cos']
    ...
    def metadata(self, df: pd.DataFrame) -> dict:
        ...
        return {'profile': self.config.profile, 'n_rows': int(len(df)), 'n_features': int(len(feature_cols)), 'target_col': self.config.target_col, 'feature_cols': feature_cols, 'ts_min': str(df[self.config.ts_col].min()), 'ts_max': str(df[self.config.ts_col].max()), 'config': asdict(self.config)}