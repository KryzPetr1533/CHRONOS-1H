    ...
    def make(model_name: str, seed: int = 42) -> ClassifierMixin:
        ...
        if name == "logreg":
            return Pipeline([
                ...
                ("model", LogisticRegression(max_iter=2000, random_state=seed, n_jobs=-1)),
            ])
...
class ClassificationTrainer:
    ...
    def run(self) -> dict[str, Any]:
        ...
        # Fit LabelMaker on TRAIN only — no leakage
        ...
        # Drop rows where label is NaN (dead-zone / insufficient look-ahead)
        ...
        for split_name, (split_df, y_series) in labeled.items():
            ...
            results_split[split_name] = {
                "n_rows": int(mask.sum()),
                "dropped_rows": int((~mask).sum()),
            }

        # Multi-class override for catboost/lgbm if needed
        ...
        # Models trained with log-loss (LogReg, CatBoost Logloss, LightGBM binary) are
        # naturally calibrated — no post-hoc calibration needed for sklearn 1.8+.
        best_model = search["best_estimator_"]

        # Baselines (majority class and persistence of previous)
        ...
        for split_name in ("train", "val", "test"):
            ...
            metrics[split_name] = evaluate_classification(
                y, y_pred, proba,
                class_names=label_maker.class_names(),
                return_col=ret_col,
                abstention_threshold=self.config.abstention_threshold,
                top_k_pct=self.config.top_k_pct,
            )

            if split_name == "test":
                proba_df = pd.DataFrame(
                    proba,
                    columns=[f"proba_{c}" for c in label_maker.class_names()],
                )
        ...
        result = {
            ...
            "label_maker": label_maker.describe(),
            "feature_cols": feature_cols,
            "n_classes": n_classes,
            "best_params": search["best_params_"],
            "cv_best_score": float(search["best_score_"]),
            "split_sizes": results_split,
            "baselines": baselines,
            "metrics": metrics,
        }
    ...
    def _select_feature_cols(self, train_df: pd.DataFrame, drop_set: set, min_non_null_frac: float = 0.05) -> List[str]:
        """Drop columns that are empty or nearly-all-null in the training split."""
        cols = []
    ...
    def _compute_baselines(self, labeled, feature_cols, label_maker) -> dict:
        ...
        for split_name in ("val", "test"):
            ...
            baselines[split_name] = {
                "majority_class": evaluate_classification(
                    y, y_maj, proba_maj,
                    class_names=label_maker.class_names(),
                ),
            }
            ...
            if _ret_col in split_df.columns and n_classes == 2:
                ...
                baselines[split_name]["persistence"] = evaluate_classification(
                    y, y_persist, proba_persist,
                    class_names=label_maker.class_names(),
                )
    ...
    def _save_artifacts(self, result, model, preds_test, label_maker, X_test, y_test) -> None:
        ...
        (out / f"{name}_{family}_metrics.json").write_text(
