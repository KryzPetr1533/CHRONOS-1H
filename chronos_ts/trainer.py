...
class TrainConfig:
    data_csv: str
    output_dir: str
    model_name: str = "ridge"  # ridge | enet | huber | catboost | lightgbm | xgboost
    ts_col: str = "ts"
...
class ModelFactory:
    ...
    def default_param_grid(model_name: str) -> dict[str, list[Any]]:
        ...
        if model_name == "huber":
            return {"model__epsilon": [1.1, 1.2, 1.35, 1.5], "model__alpha": [1e-5, 1e-4, 1e-3]}
        if model_name == "catboost":
            return {
                "depth": [4, 6, 8],
...
class TabularTrainer:
    ...
    def run(self) -> dict[str, Any]:
        ...
        search = self._manual_grid_search(
            estimator=estimator,
            param_grid=grid,
            X=X_train,
            y=y_train,
        ...
        result = {
            ...
            "n_rows": {k: int(len(v)) for k, v in splits.items()},
            "baseline": {
                "val": evaluate_predictions(y_val, y_val_baseline),
                "test": evaluate_predictions(y_test, y_test_baseline),
            },
            "model": {
                "val": evaluate_predictions(y_val, y_val_pred),
                "test": evaluate_predictions(y_test, y_test_pred),
            },
            "ts_range": {
                split_name: {
                    "min": str(split_df[self.config.ts_col].min()),
                    "max": str(split_df[self.config.ts_col].max()),
                }
        ...
        pd.DataFrame(
            {
                self.config.ts_col: splits["test"][self.config.ts_col],
                "y_true": y_test,
                "y_pred": y_test_pred,
                "y_pred_baseline": y_test_baseline,
            }
