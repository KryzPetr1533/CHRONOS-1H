"""
Event bar builders: convert 1m klines to time / volume / dollar bars.

Usage:
    from chronos_ts.bars import VolumeBarBuilder, DollarBarBuilder
    vb = VolumeBarBuilder()           # threshold auto-calibrated
    bars = vb.build(df_1m)            # returns pd.DataFrame with standard schema

All builders produce a strictly causal, non-overlapping bar series.
Schema: bar_start, bar_end, open, high, low, close, volume, dollar_volume,
        n_trades, taker_buy_base, dt_seconds
"""
...
BAR_COLS = [
    ...
    "volume", "dollar_volume", "n_trades", "taker_buy_base", "dt_seconds",
]
...
class _BarBuilder:
    """Shared conversion and validation utilities."""
    ...
    def _finalize_bar(self, rows: list, last_bar_end: pd.Timestamp) -> pd.DataFrame:
        ...
        # Leakage assertion: bar_end must be <= last_bar_end of source
...
class TimeBarBuilder(_BarBuilder):
    """
    Resample 1m klines to a fixed frequency (default 1h).
    This is equivalent to the existing 1h data but re-derived from 1m.
    """
    ...
    def build(self, df_1m: pd.DataFrame) -> pd.DataFrame:
        ...
        agg = df.resample(self.freq).agg(
            open=("open", "first"),
            high=("high", "max"),
            low=("low", "min"),
            close=("close", "last"),
            volume=("volume", "sum"),
            dollar_volume=("dollar_volume" if "dollar_volume" in df.columns
                           else "volume", "sum"),
            n_trades=("n_trades" if "n_trades" in df.columns else "volume", "count"),
            taker_buy_base=("taker_buy_base" if "taker_buy_base" in df.columns
                            else "volume", "sum"),
        ).dropna(subset=["open"]).reset_index()

        bars = pd.DataFrame({
            "bar_start":     agg["ts"],
            "bar_end":       agg["ts"] + pd.Timedelta(self.freq),
            "open":          agg["open"],
            "high":          agg["high"],
            "low":           agg["low"],
            "close":         agg["close"],
            "volume":        agg["volume"],
            "dollar_volume": agg.get("dollar_volume", agg["volume"] * agg["close"]),
            "n_trades":      agg["n_trades"],
            "taker_buy_base": agg["taker_buy_base"],
            "dt_seconds":    pd.Timedelta(self.freq).total_seconds(),
        })
        return bars
...
class VolumeBarBuilder(_BarBuilder):
    """
    Close a bar when cumulative volume >= threshold.

    Parameters
    ----------
    volume_threshold : target volume per bar. If None, auto-calibrate so that
                       average bar duration ≈ 1 hour.
    """
    ...
    def _calibrate(self, df: pd.DataFrame) -> float:
        """Average volume per 60 1m bars ≈ 1h average volume."""
    ...
    def _accumulate(self, df: pd.DataFrame, threshold: float, bars: list, kind: str) -> None:
        ...
        for _, row in df.iterrows():
            ...
            if limit >= threshold:
                ...
                bars.append([
                    bar_start, row["ts"], bar_open, bar_high, bar_low,
                    price, cum_vol, cum_dollar, int(cum_trades), cum_buy, dt_s,
                ])
                cum_vol = cum_dollar = cum_trades = cum_buy = 0.0
                bar_start = None
...
class DollarBarBuilder(VolumeBarBuilder):
    """
    Close a bar when cumulative dollar_volume >= threshold.
    Inherits VolumeBarBuilder; only the calibration and accumulation key differ.
    """
...
# ------------------------------------------------------------------ #
# ImbalanceBarBuilder (simplified tick-imbalance variant)
# ------------------------------------------------------------------ #

class ImbalanceBarBuilder(_BarBuilder):
    """
    Close a bar when cumulative signed-volume imbalance exceeds threshold.
    Uses buy_vol > sell_vol direction from taker_buy_base.
    """
    ...
    def build(self, df_1m: pd.DataFrame) -> pd.DataFrame:
        ...
        imbalance = (buy_vol - sell_vol)  # signed
