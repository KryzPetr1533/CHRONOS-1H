...
def flatten_backtests(pred_list, tgt_list, pred_col="mean"):
    ...
    for pred, tgt in zip(pred_list, tgt_list):
        ...
        merged = pred_df[["item_id", "timestamp", pred_col]].merge(
            tgt_tail[["item_id", "timestamp", "target"]],
...
def main() -> None:
    ...
    # number of one-step backtest windows in held-out test tail
    ...
    predictor.fit(
        ...
        hyperparameters={
            "Chronos2": [
                {
                    "ag_args": {"name_suffix": "SmallZeroShot"},
    ...
    # aligned rolling one-step forecasts on the held-out tail
    targets = predictor.backtest_targets(
        full_ts,
        num_val_windows=n_test_windows,
        val_step_size=val_step_size,
    )

    for model_name in model_names:
        print(f"backtesting model: {model_name}")
        preds = predictor.backtest_predictions(
            full_ts,
            model=model_name,
            num_val_windows=n_test_windows,
            val_step_size=val_step_size,
        ...
        # naive baseline = previous observed return at the same timestamp row
        ...
        merged.rename(columns={"mean": "y_pred", "target": "y_true", "log_ret_1h": "y_pred_baseline"}).to_csv(
            OUT_DIR / f"{model_name}_backtest_predictions.csv", index=False
        )
    ...
    final = {
        ...
        "n_test_backtest_windows": int(n_test_windows),
        "model_results": results,
        "model_names": model_names,
    }

    (OUT_DIR / "chronos2_metrics.json").write_text(
