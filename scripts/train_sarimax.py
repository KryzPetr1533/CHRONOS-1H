...
def fit_and_forecast(
    ...
    with warnings.catch_warnings():
        ...
        model = SARIMAX(
            endog=train_df["target_log_ret_1h"].astype(float),
            exog=exog_train,
            order=order,
            trend=trend,
...
def main() -> None:
    ...
    # naive baseline = previous observed return
    ...
    # AR-only and ARX/SARIMAX-like variants
    candidate_sets = {
        "ar_only": [],
        "arx_exog": exog_cols,
    }
    ...
    # refit best on train+val, then forecast test
    df_trainval = pd.concat([df_train, df_val], axis=0).reset_index(drop=True)
    pred_test = fit_and_forecast(
        train_df=df_trainval,
        future_df=df_test,
        order=best["order"],
        trend=best["trend"],
        exog_cols=best["exog_cols"],
    )
    ...
    pd.DataFrame(search_rows).sort_values("val_rmse").to_csv(
        OUT_DIR / "sarimax_search_results.csv", index=False
    )
    ...
    ).to_csv(OUT_DIR / "sarimax_test_predictions.csv", index=False)

    (OUT_DIR / "sarimax_metrics.json").write_text(
