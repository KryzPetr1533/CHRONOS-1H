...
class TokenTransformerRunner:
    ...
    def __init__(self, cfg: Optional[TokenTransformerConfig]=None, **kwargs):
        self.cfg = cfg or TokenTransformerConfig(**{k: v for k, v in kwargs.items() if k in TokenTransformerConfig.__dataclass_fields__})
    ...
    def run(self) -> dict[str, Any]:
        ...
        model = _GRUTokenModel(vocab_size=c.n_tokens, hidden=c.hidden, n_layers=c.n_layers, dropout=c.dropout).to(device)
        ...
        for split_name, X, y in [('val', X_va, y_va), ('test', X_te, y_te)]:
            ...
            print(f"[{split_name}]  balanced_acc={m['balanced_accuracy']:.4f}  mcc={m['mcc']:.4f}  roc_auc={m['roc_auc']:.4f}")
        result = {'config': vars(c), 'label_maker': lm.describe(), 'metrics': metrics, 'best_val_loss': float(best_val_loss), 'n_params': sum((p.numel() for p in model.parameters()))}
        out = Path(c.output_dir) / 'token_transformer'
        out.mkdir(parents=True, exist_ok=True)
        (out / 'token_transformer_metrics.json').write_text(json.dumps(result, indent=2, default=str), encoding='utf-8')
    ...
    def __new__(cls, vocab_size, hidden, n_layers, dropout):
        ...
        class GRUModel(nn.Module):
            ...
            def __init__(self):
                ...
                self.gru = nn.GRU(hidden // 2, hidden, num_layers=n_layers, batch_first=True, dropout=dropout if n_layers > 1 else 0)
...
def main() -> None:
    ...
    cfg = TokenTransformerConfig(data_csv=args.data_csv, output_dir=args.output_dir, n_tokens=args.n_tokens, lookback=args.lookback, hidden=args.hidden, epochs=args.epochs, seed=args.seed)
