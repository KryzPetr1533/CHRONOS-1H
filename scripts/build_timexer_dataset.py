...
def main() -> None:
    ...
    # keep target as regression target
    out = pd.DataFrame({
        "series_id": "BTCUSDT",
        "time_idx": np.arange(len(df), dtype=int),
        "ds": pd.to_datetime(df["ts"], utc=True).dt.tz_convert(None),
        "target": df["target_log_ret_1h"].astype(float),

        # past / unknown-at-forecast-time features
        "log_ret_1h": df["log_ret_1h"].astype(float),
        "premium_chg": df["premium_chg"].astype(float),
        "fundingRate": df["fundingRate"].astype(float),
        "vol_chg": df["vol_chg"].astype(float),
        "trades_chg": df["trades_chg"].astype(float),
        "taker_buy_share": df["taker_buy_share"].astype(float),
        "rv_24": df["rv_24"].astype(float),

        # known future calendar features
        "hour_sin": df["hour_sin"].astype(float),
        "hour_cos": df["hour_cos"].astype(float),
        "dow_sin": df["dow_sin"].astype(float),
        "dow_cos": df["dow_cos"].astype(float),
    })
    ...
    meta = {
        ...
        "ds_min": str(out["ds"].min()) if len(out) else None,
        "ds_max": str(out["ds"].max()) if len(out) else None,
    }
