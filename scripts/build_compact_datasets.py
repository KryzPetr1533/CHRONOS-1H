...
def save_dataset(df: pd.DataFrame, name: str, feature_cols: list[str]) -> None:
    ...
    # funding is sparse/event-like; forward fill is acceptable for classical exog use
    ...
    meta = {
        ...
        "ts_min": str(out["ts"].min()) if len(out) else None,
        "ts_max": str(out["ts"].max()) if len(out) else None,
    }
...
def main() -> None:
    ...
    for name, cols in [
        ("btcusdt_core_mean_small", mean_small),
        ("btcusdt_core_vol_small", vol_small),
        ("btcusdt_core_seq_small", seq_small),
    ]:
