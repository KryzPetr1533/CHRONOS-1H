"""
Build event-bar feature datasets (volume bars & dollar bars).

Two modes:
  1. From 1m klines (accurate): requires data/raw_fine/ populated by fetch_fine_data.py
  2. From existing 1h OHLCV (proxy): synthesises a bar-like dataset for testing the
     pipeline when 1m data is not yet available. This is clearly labelled as a proxy
     and not used for final results.

CLI:
    python scripts/build_event_dataset.py                      # auto-detect mode
    python scripts/build_event_dataset.py --from-1h            # force 1h proxy mode
    python scripts/build_event_dataset.py --from-1m            # require 1m klines

Notebook / script import:
    from scripts.build_event_dataset import EventDatasetBuilder
    builder = EventDatasetBuilder()
    vb_df, db_df = builder.run()
"""
...
class EventDatasetBuilder:
    """
    Build volume-bar and dollar-bar feature matrices.
    ...
    def __init__(
        self,
        raw_1m_dir: str = "data/raw_fine/BTCUSDT/1m",
        source_1h_csv: str = "data/btcusdt_1h_merged.csv",
    ...
    def run(self, force_proxy: bool = False, force_1m: bool = False) -> Tuple[pd.DataFrame, pd.DataFrame]:
        """
        Build and save volume-bar and dollar-bar datasets.

        Returns (volume_bars_df, dollar_bars_df) both with features + target.
        """
        use_proxy = self._should_use_proxy(force_proxy, force_1m)

        if use_proxy:
            print("Mode: 1h-proxy (no 1m klines found). "
                  "Run `python scripts/fetch_fine_data.py` to get real data.")
        ...
        feat_builder = EventFeatureBuilder(EventFeatureConfig(
            lag_bars=[1, 2, 3, 6, 12, 24],
        ...
        # Add classification targets using the same label framework
        vb_feat = self._add_targets(vb_feat, ts_col="bar_end")
        db_feat = self._add_targets(db_feat, ts_col="bar_end")

        # Drop early rows that lack full lag history
    ...
    def _synthesise_1m_from_1h(self) -> pd.DataFrame:
        """
        PROXY ONLY: decompose each 1h bar into 60 synthetic 1m bars.
        Preserves OHLCV totals (volume split uniformly, price random walk within bar).
        NOT suitable for production — use only for pipeline testing.
        """
        ...
        for _, row in df_1h.iterrows():
            ...
            for i in range(60):
                ...
                rows.append({
                    "ts": minute_ts,
                    "open": p, "high": p * (1 + rng.uniform(0, 0.0002)),
                    "low":  p * (1 - rng.uniform(0, 0.0002)), "close": p,
                    "volume": v,
                    "dollar_volume": v * p,
                    "n_trades": max(1, n_trades // 60),
                    "taker_buy_base": buy_split[i],
                })
    ...
    def _add_targets(self, df: pd.DataFrame, ts_col: str) -> pd.DataFrame:
        """Add log_ret_bar (already present from EventFeatureBuilder) and vol_regime label."""
        # Ensure log_ret_bar exists
    ...
    def _write_meta(self, df: pd.DataFrame, path: Path, bar_type: str, is_proxy: bool) -> None:
        meta = {
            "bar_type": bar_type,
            "is_proxy_from_1h": is_proxy,
            "n_rows": len(df),
            "n_cols": df.shape[1],
            "ts_min": str(df["bar_end"].min() if "bar_end" in df.columns else ""),
            "ts_max": str(df["bar_end"].max() if "bar_end" in df.columns else ""),
            "note": (
                "PROXY dataset: 1h bars decomposed into synthetic 1m bars. "
                "For real results run scripts/fetch_fine_data.py first."
            ) if is_proxy else "Real 1m klines from Binance FAPI",
        }
...
def main() -> None:
    ...
    EventDatasetBuilder(output_dir=args.output_dir).run(
        force_proxy=args.from_1h,
        force_1m=args.from_1m,
    )
