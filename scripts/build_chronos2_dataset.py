...
PAST_COVS = ['log_ret_1h', 'premium_chg', 'fundingRate', 'vol_chg', 'trades_chg', 'taker_buy_share', 'rv_24', 'fundingRate_missing', 'taker_buy_share_missing']
...
def main() -> None:
    ...
    for path in sorted(paths):
        ...
        out = pd.DataFrame({'item_id': item_id, 'timestamp': pd.to_datetime(df['ds'], utc=True).dt.tz_convert(None), 'target': pd.to_numeric(df['target'], errors='coerce')})
    ...
    meta = {'csv': str(OUT_CSV), 'n_rows': int(len(panel)), 'n_items': int(panel['item_id'].nunique()), 'items': sorted(panel['item_id'].unique().tolist())[:50], 'known_covariates_names': KNOWN_COVS, 'past_covariates': PAST_COVS, 'timestamp_min': str(panel['timestamp'].min()), 'timestamp_max': str(panel['timestamp'].max())}
