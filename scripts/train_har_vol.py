...
def main() -> None:
    ...
    # naive baseline: today's abs return predicts next-hour abs return
    ...
    result = {
        ...
        "baseline": {
            "val": regression_metrics(y_val, y_val_baseline),
            "test": regression_metrics(y_test, y_test_baseline),
        },
        "model": {
    ...
    pd.DataFrame(search_rows).sort_values("val_rmse").to_csv(
        OUT_DIR / "har_search_results.csv", index=False
    )
    pd.DataFrame(
        {
            "ts": df_test["ts"],
    ...
    ).to_csv(OUT_DIR / "har_test_predictions.csv", index=False)

    (OUT_DIR / "har_metrics.json").write_text(
