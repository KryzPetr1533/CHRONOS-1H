"""
Hydra-configured CLI training with full MLflow logging.
Bonus deliverable for Task 2 Phase 2.

Usage:
    # Default run (vol_regime + catboost, no PRD promotion)
    python scripts/train_mlflow.py

    # Final run: promote to PRD
    python scripts/train_mlflow.py experiment=final mlflow.promote_to_prd=true

    # Override anything
    python scripts/train_mlflow.py model=logreg label=direction seed=123

    # Multirun sweep — one MLflow run per combination
    python scripts/train_mlflow.py -m label=vol_regime,large_move model=logreg,catboost

Notebook / script import (no Hydra):
    from scripts.train_mlflow import TrainWithMLflow, MLflowRunConfig
    result = TrainWithMLflow().run_direct(
        label_family='vol_regime',
        model_name='catboost',
        promote_to_prd=False,
    )
"""
...
class TrainWithMLflow:
    """
    Combines ClassifierRunner + MLflow logging.
    Callable from both Hydra CLI and notebook/plain scripts.
    """
    ...
    def run_direct(
        ...
        output_base: str = "outputs/models/clf",
        mlflow_cfg: Optional[MLflowRunConfig] = None,
        seed: int = 42,
        cv_splits: int = 3,
        ...
        """
        Train + log to MLflow without Hydra.
        Convenience method for notebooks and plain scripts.
        """
        ...
        configure_mlflow(
            tracking_uri=cfg_mlflow.tracking_uri,
            experiment_name=cfg_mlflow.experiment_name,
            s3_endpoint_url=cfg_mlflow.s3_endpoint_url,
            aws_access_key_id=cfg_mlflow.aws_access_key_id,
            aws_secret_access_key=cfg_mlflow.aws_secret_access_key,
        )
        ...
        return ClassifierRunner(run_cfg).run(
            mlflow_experiment=cfg_mlflow.experiment_name,
            register_as=cfg_mlflow.registered_model_name,
            promote_to_prd=cfg_mlflow.promote_to_prd,
        )

    def run_from_hydra(self, cfg) -> dict[str, Any]:
        """Called by the Hydra main() below."""
        import hydra
        ...
        ml = cfg.mlflow
        configure_mlflow(
            tracking_uri=ml.tracking_uri,
            experiment_name=ml.experiment_name,
            s3_endpoint_url=ml.s3_endpoint_url,
            aws_access_key_id=ml.aws_access_key_id,
            aws_secret_access_key=ml.aws_secret_access_key,
        )
        ...
        result = ClassifierRunner(run_cfg).run(
            mlflow_experiment=ml.experiment_name,
            register_as=ml.registered_model_name,
            promote_to_prd=bool(ml.promote_to_prd),
        )

        # Log the resolved Hydra config as an artifact
        try:
            import mlflow
