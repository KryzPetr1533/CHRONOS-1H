...
class PrdPredictor:
    ...
    def predict(self, input_csv: str='outputs/datasets/btcusdt_clf_core.csv', n_rows: int=10) -> pd.DataFrame:
        ...
        split_cfg = TimeRangeSplitConfig(0.7, 0.15, 0.15)
...
try:
    ...
    def main(cfg: DictConfig) -> None:
        pred_cfg = PrdPredictorConfig(registered_model_name=cfg.mlflow.registered_model_name, tracking_uri=cfg.mlflow.tracking_uri, s3_endpoint_url=cfg.mlflow.s3_endpoint_url, aws_access_key_id=cfg.mlflow.aws_access_key_id, aws_secret_access_key=cfg.mlflow.aws_secret_access_key)
        predictor = PrdPredictor(pred_cfg)
        import hydra as _h
        project_root = Path(_h.utils.get_original_cwd())
        result = predictor.predict(input_csv=str(project_root / cfg.predict.input_csv), n_rows=int(cfg.predict.n_rows))
