...
DATA_CSV = Path("outputs/datasets/btcusdt_core_seq_small.csv")
OUT_DIR = Path("outputs/models/seq_core_small")
...
def regression_metrics(y_true: np.ndarray, y_pred: np.ndarray) -> dict[str, float]:
    ...
    top_dir_acc = (
        float((sign_true[top_mask] == sign_pred[top_mask]).mean())
        if top_mask.sum() > 0 else np.nan
    )
...
def main() -> None:
    ...
    search_space = [
        {"model_kind": "gru", "lookback": 48, "hidden_dim": 32, "dropout": 0.0},
        {"model_kind": "gru", "lookback": 168, "hidden_dim": 32, "dropout": 0.0},
        {"model_kind": "lstm", "lookback": 48, "hidden_dim": 32, "dropout": 0.0},
        {"model_kind": "lstm", "lookback": 168, "hidden_dim": 32, "dropout": 0.0},
    ]
    ...
    for cfg_row in search_space:
        ...
        model = RNNRegressor(
            ...
            hidden_dim=cfg_row["hidden_dim"],
            model_kind=cfg_row["model_kind"],
            dropout=cfg_row["dropout"],
        ).to(device)

        print("\ntraining", cfg_row)
        model, val_rmse, val_pred = train_one_model(
            model=model,
        ...
        if val_metrics["rmse"] < best_val_rmse:
            ...
            # naive baseline on same aligned positions
    ...
    result = {
        ...
        "n_rows": {
            "total": int(len(df)),
            "train_target_positions": int(train_end),
            "val_target_positions": int(len(best["val_positions"])),
            "test_target_positions": int(len(best["test_positions"])),
        },
        "feature_cols": best["feature_cols"],
        "baseline": {
            "val": regression_metrics(best["val_true"], best["baseline_val"]),
            "test": regression_metrics(best["test_true"], best["baseline_test"]),
        },
        "model": {
            "val": regression_metrics(best["val_true"], best["val_pred"]),
            "test": regression_metrics(best["test_true"], best["test_pred"]),
        },
        "ts_range": {
            "train": {"min": str(df.iloc[:train_end]["ts"].min()), "max": str(df.iloc[:train_end]["ts"].max())},
            "val": {"min": str(df.iloc[train_end:val_end]["ts"].min()), "max": str(df.iloc[train_end:val_end]["ts"].max())},
            "test": {"min": str(df.iloc[val_end:]["ts"].min()), "max": str(df.iloc[val_end:]["ts"].max())},
        },
    }

    pd.DataFrame(search_rows).sort_values("val_rmse").to_csv(
        OUT_DIR / "seq_search_results.csv", index=False
    )

    pd.DataFrame(
        {
            "ts": df.iloc[best["test_positions"]]["ts"].to_numpy(),
            "y_true": best["test_true"],
            "y_pred": best["test_pred"],
            "y_pred_baseline": best["baseline_test"],
        }
    ).to_csv(OUT_DIR / "seq_test_predictions.csv", index=False)

    (OUT_DIR / "seq_metrics.json").write_text(
