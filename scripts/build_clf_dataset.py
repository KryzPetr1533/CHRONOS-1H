"""
Build the classification-ready dataset.

CLI:
    python scripts/build_clf_dataset.py
    python scripts/build_clf_dataset.py --input data/btcusdt_1h_merged.csv --output outputs/datasets/btcusdt_clf_core.csv

Notebook / script import:
    from scripts.build_clf_dataset import ClfDatasetBuilder
    builder = ClfDatasetBuilder()
    df = builder.run()                   # build + sanity + label preview
    df = builder.build_only()            # build only, no check
    builder.sanity_check(df)
    builder.preview_labels(df)
"""
...
class ClfDatasetBuilder:
    """
    Builds and validates the classification feature matrix.

    Parameters
    ----------
    input_csv  : raw merged Binance CSV (default: data/btcusdt_1h_merged.csv)
    output_csv : where to save the feature matrix
    """

    def __init__(
        self,
        input_csv: str = "data/btcusdt_1h_merged.csv",
        output_csv: str = "outputs/datasets/btcusdt_clf_core.csv",
    ...
    def run(self) -> pd.DataFrame:
        """Build feature matrix, run sanity check, preview labels, save."""
        df = self.build_only()
    ...
    def build_only(self) -> pd.DataFrame:
        """Run feature engineering and save. No checks."""
    ...
    def sanity_check(self, df: pd.DataFrame) -> None:
        """
        Verify the dataset is safe to use for training.
        Raises SystemExit on any critical error.
        """
        ...
        high_null = null_frac[null_frac > 0.10]
        if len(high_null):
    ...
    def preview_labels(self, df: pd.DataFrame) -> list[dict]:
        """
        Show class distribution for all label families.
        Fits LabelMaker on the training split only (70%) — no leakage.
        Saves outputs/datasets/label_preview.json.

        Returns
        -------
        List of dicts with per-family stats.
        """
        ...
        families = [
            LabelConfig(target_family="direction"),
            LabelConfig(target_family="large_move"),
            LabelConfig(target_family="vol_regime"),
            LabelConfig(target_family="horizon_dir", horizon=6),
            LabelConfig(target_family="return_token", n_tokens=5),
        ]
        ...
        for cfg in families:
            ...
            row = {
                ...
                "train_base_rates": {
                    lm.class_names()[i]: float(c / train_counts.sum())
                    for i, c in enumerate(train_counts)
                },
                "test_base_rates": {
                    lm.class_names()[i]: float(c / test_counts.sum())
                    for i, c in enumerate(test_counts)
                },
                "edges": lm._edges.tolist() if lm._edges is not None else None,
            }
            rows.append(row)
