...
def s3_endpoint(cfg: dict[str, str], override: str | None=None) -> str:
    if override:
        return override
    if (url := os.environ.get('MLFLOW_S3_ENDPOINT_URL')):
        return url
    port = cfg.get('S3_API_PORT', '9000')
    return f'http://localhost:{port}'

def make_client(cfg: dict[str, str], endpoint_override: str | None=None):
    return boto3.client('s3', endpoint_url=s3_endpoint(cfg, endpoint_override), aws_access_key_id=cfg['MINIO_ROOT_USER'], aws_secret_access_key=cfg['MINIO_ROOT_PASSWORD'], region_name='us-east-1')
...
def upload_tree(s3, bucket: str, s3_prefix: str, local_dir: Path, *, dry_run: bool=False) -> int:
...
def cmd_upload(args: argparse.Namespace) -> int:
    ...
    endpoint = s3_endpoint(cfg, args.endpoint)
    print(f'Endpoint : {endpoint}')
    print(f'Source   : {src}/')
    print(f'Dest     : s3://{bucket}/{prefix}/')
    s3 = make_client(cfg, args.endpoint)
    if not args.dry_run:
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description='Upload datasets to MinIO (S3)')
    p.add_argument('--env-file', type=Path, default=DEFAULT_ENV_FILE, help=f'Path to mlflow .env (default: {DEFAULT_ENV_FILE.relative_to(REPO_ROOT)})')
    p.add_argument('--list', action='store_true', help='List objects under --prefix instead of uploading')
    p.add_argument('--src', type=Path, default=DEFAULT_SRC, help='Local directory to upload')
    p.add_argument('--tag', default=None, help="Version folder under prefix (default: today's UTC date YYYYMMDD)")
    p.add_argument('--prefix', default=DEFAULT_PREFIX, help='S3 key prefix')
    p.add_argument('--endpoint', default=None, help='S3 endpoint URL (default: MLFLOW_S3_ENDPOINT_URL env, else localhost:S3_API_PORT)')
