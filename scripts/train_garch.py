...
def make_arch_model(kind: str, y_scaled: pd.Series):
    kind = kind.lower()
    if kind == 'garch':
        return arch_model(y_scaled, mean='Zero', vol='GARCH', p=1, o=0, q=1, dist='normal', rescale=False)
    if kind == 'gjr':
        return arch_model(y_scaled, mean='Zero', vol='GARCH', p=1, o=1, q=1, dist='normal', rescale=False)
    if kind == 'egarch':
        return arch_model(y_scaled, mean='Zero', vol='EGARCH', p=1, o=1, q=1, dist='normal', rescale=False)
...
def next_variance(kind: str, params: pd.Series, prev_eps: float, prev_sigma2: float) -> float:
    ...
    if kind == 'garch':
        ...
        log_sigma2_next = omega + beta * np.log(prev_sigma2) + alpha * (abs(z_prev) - expected_abs_z) + gamma * z_prev
        sigma2_next = float(np.exp(log_sigma2_next))
    else:
        raise ValueError(f'Unsupported model kind: {kind}')
    return max(float(sigma2_next), eps)

def forecast_sigma_path(kind: str, params: pd.Series, prev_eps: float, prev_sigma2: float, eval_returns_scaled: np.ndarray) -> tuple[np.ndarray, float, float]:
    preds = []
...
def main() -> None:
    ...
    for kind in candidates:
        try:
            ...
            pred_val, _, _ = forecast_sigma_path(kind=kind, params=params, prev_eps=last_eps, prev_sigma2=last_sigma2, eval_returns_scaled=val_scaled)
            val_rmse = float(np.sqrt(mean_squared_error(y_val, pred_val)))
            row = {'model_kind': kind, 'val_rmse': val_rmse, 'val_mae': float(mean_absolute_error(y_val, pred_val))}
            search_rows.append(row)
            if val_rmse < best_val_rmse:
                best_val_rmse = val_rmse
                best = {'kind': kind, 'val_pred': pred_val}
    ...
    pred_test, _, _ = forecast_sigma_path(kind=best['kind'], params=best_params, prev_eps=best_last_eps, prev_sigma2=best_last_sigma2, eval_returns_scaled=test_scaled)
    result = {'data_csv': str(DATA_CSV), 'best_model': best['kind'], 'split': asdict(split_cfg), 'n_rows': {'train': int(len(df_train)), 'val': int(len(df_val)), 'test': int(len(df_test))}, 'baseline': {'val': regression_metrics(y_val, baseline_val), 'test': regression_metrics(y_test, baseline_test)}, 'model': {'val': regression_metrics(y_val, best['val_pred']), 'test': regression_metrics(y_test, pred_test)}, 'ts_range': {'train': {'min': str(df_train['ts'].min()), 'max': str(df_train['ts'].max())}, 'val': {'min': str(df_val['ts'].min()), 'max': str(df_val['ts'].max())}, 'test': {'min': str(df_test['ts'].min()), 'max': str(df_test['ts'].max())}}}
    pd.DataFrame(search_rows).sort_values('val_rmse').to_csv(OUT_DIR / 'garch_search_results.csv', index=False)
    pd.DataFrame({'ts': df_test['ts'], 'y_true_abs_ret': y_test, 'y_pred_sigma': pred_test, 'y_pred_baseline': baseline_test}).to_csv(OUT_DIR / 'garch_test_predictions.csv', index=False)
    (OUT_DIR / 'garch_metrics.json').write_text(json.dumps(result, ensure_ascii=False, indent=2), encoding='utf-8')
