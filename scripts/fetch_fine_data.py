...
class FineDataFetcher:
    ...
    def __init__(self, output_dir: str='data/raw_fine', request_delay: float=0.3):
        self.output_dir = Path(output_dir)
        self.request_delay = request_delay

    def run(self, symbol: str='BTCUSDT', days: int=30, end_date: Optional[datetime]=None, fetch_trades: bool=False) -> pd.DataFrame:
        if end_date is None:
    ...
    def _fetch_day_klines(self, symbol: str, day: datetime) -> Optional[pd.DataFrame]:
        ...
        while open_time < end_ms:
            batch = self._get_klines(symbol, '1m', open_time, min(open_time + limit * 60000, end_ms), limit)
            if not batch:
                break
            rows.extend(batch)
            open_time = batch[-1][0] + 60000
            if len(batch) < limit:
                break
        ...
        return df[['ts', 'open', 'high', 'low', 'close', 'volume', 'quote_volume', 'n_trades', 'taker_buy_base', 'taker_buy_quote']].copy()

    @retry(stop=stop_after_attempt(5), wait=wait_exponential(multiplier=1, min=1, max=30))
    def _get_klines(self, symbol, interval, start_time, end_time, limit):
        r = requests.get(f'{BASE_URL}/fapi/v1/klines', params={'symbol': symbol, 'interval': interval, 'startTime': start_time, 'endTime': end_time, 'limit': limit}, timeout=15)
        r.raise_for_status()
        return r.json()
    ...
    def _fetch_aggtrades(self, symbol: str, start: datetime, end: datetime) -> None:
        ...
        while current < end_ms:
            ...
            if rows:
                ...
                df.columns = ['agg_id', 'price', 'qty', 'first_id', 'last_id', 'timestamp', 'is_buyer_maker', 'is_best_match']
    ...
    def _get_aggtrades(self, symbol, start_time, end_time):
        r = requests.get(f'{BASE_URL}/fapi/v1/aggTrades', params={'symbol': symbol, 'startTime': start_time, 'endTime': end_time, 'limit': 1000}, timeout=15)
        r.raise_for_status()
        return r.json()
    ...
    def _write_provenance(self, symbol: str, start: datetime, end: datetime, n_rows: int, fetch_trades: bool) -> None:
        prov = {'symbol': symbol, 'endpoint': f'{BASE_URL}/fapi/v1/klines', 'interval': '1m', 'start_utc': start.isoformat(), 'end_utc': end.isoformat(), 'n_rows': n_rows, 'aggtrades_fetched': fetch_trades, 'pulled_at': datetime.now(timezone.utc).isoformat()}
